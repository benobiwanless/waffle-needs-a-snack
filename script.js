...
let currentPage = 0;
function showCover() { reader.classList.remove("active"); cover.classList.add("active"); }
function showReader() { cover.classList.remove("active"); reader.classList.add("active"); renderPage(); }
function renderPage() {
...
startBtn.addEventListener("click", () => { currentPage = 0; showReader(); });
homeBtn.addEventListener("click", showCover);
prevBtn.addEventListener("click", () => { if (currentPage > 0) { currentPage--; renderPage(); } });
nextBtn.addEventListener("click", () => { if (currentPage < BOOK_PAGES.length - 1) { currentPage++; renderPage(); } else { showCover(); } });
